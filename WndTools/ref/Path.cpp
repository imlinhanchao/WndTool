...
CString Path::Folder( HWND hWnd, CString sRootPath/*=_T("")*/ )
{
	...
	_tcscpy_s(szBuffer, MAX_PATH, sRootPath.GetBuffer());
...
int Path::CopyTo(std::vector<CString> lstSrc, CString sDst )
{
	CString sSrc;
	for (std::vector<CString>::iterator it = lstSrc.begin(); it != lstSrc.end(); it++)
	{
		sSrc += it->Trim('\\') + _T("|");
	}
	sDst.Trim('\\');
	...
	_tcscpy_s(pFrom, nSrcSize, sSrc.GetBuffer());
	_tcscpy_s(pTo, nDstSize, sDst.GetBuffer());
	sSrc.ReleaseBuffer();
	sDst.ReleaseBuffer();
...
std::vector<CString> Path::Traversing( CString sDirectory, FILITER filter/*=NULL*/ )
{
	std::vector<CString> lstPath;
	...
	if(!sDirectory.IsEmpty() && Exists(sDirectory) && !IsEmpty(sDirectory))
	{
		...
		{     
			...
			{      
				...
				std::vector<CString> lstPathEx = Traversing(sPath, filter);
...
std::vector<CString> Path::GetFileList( CString sDirectory, CString sFormat/*=_T("*")*/ )
{
	std::vector<CString> lstPath;
